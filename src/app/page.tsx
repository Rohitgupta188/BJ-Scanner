...
export default function DashboardPage() {
  ...
  const handleQtyDelta = (sku: string, delta: number) => {
    const current = readStorage();
    const target = current.find(i => i.sku === sku);
    if (!target) return;
    const qty = target.qty + delta;
    const updated = qty <= 0
      ? current.filter(i => i.sku !== sku)
      : current.map(i => i.sku === sku ? { ...i, qty } : i);
    writeStorage(updated);
    setItems(updated);
    if (qty <= 0) addToast('Item removed', 'success');
  };
    ...
    <>
      ...
      <div style={{ position: 'relative', zIndex: 1, minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
        ...
        <header className="portal-header">
          ...
          <h1 style={{ fontFamily: '"DM Serif Display",  serif', fontSize: '2.25rem', fontWeight: 400, color: 'var(--text-primary)', marginBottom: '0.5rem', letterSpacing: '-0.02em' }}>
            QR Scanner
          </h1>
                ...
                <>
                  <div className="table-wrap" style={{ overflowX: 'auto' }}>
                    <table className="scan-table">
                            ...
                            <tr key={item.sku} className="fade-in">
                              ...
                              <td>
                                <TableSelect
                                  ...
                                  onChange={v => handleItemFieldChange(item.sku, 'metalPurity', v as number)}
                                />
                              </td>
                              <td>
                                <TableSelect
                                  ...
                                  onChange={v => handleItemFieldChange(item.sku, 'metalType', v as string)}
                                />
                              </td>
                      ...
                      <tfoot>
                        <tr style={{ borderTop: '1px solid var(--border-bright)', background: 'rgba(184,151,90,0.05)' }}>
                  ...
                  <div className="item-cards">
                    {items.map(item => (
                      <div key={item.sku} className="item-card fade-in">
                        <div className="item-card-top">
                          {item.imageUrl ? (
                            // eslint-disable-next-line @next/next/no-img-element
                            <img src={item.imageUrl} alt={item.designNumber} className="item-card-img" />
                          ) : (
                            <div className="item-card-img item-card-img-empty">—</div>
                          )}
                          <div className="item-card-info">
                            <span className="item-card-design">{item.designNumber}</span>
                            <span className="item-card-weights">
                              G {item.grossWeight.toFixed(3)} · N {item.netWeight.toFixed(3)}
                              {item.stoneWeight > 0 && ` · S ${item.stoneWeight.toFixed(3)}`}
                            </span>
                          </div>
                          <button
                            className="btn btn-ghost item-card-remove"
                            onClick={() => handleRemoveItem(item.sku)}
                          >
                            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                              <polyline points="3 6 5 6 21 6" /><path d="M19 6l-1 14H6L5 6" />
                        ...
                        <div className="item-card-controls">
                          ...
                          <div className="qty-stepper">
                            <button onClick={() => handleQtyDelta(item.sku, -1)}>−</button>
                            <span>{item.qty}</span>
                            <button onClick={() => handleQtyDelta(item.sku, +1)}>+</button>
                          </div>
                          <span className="item-card-total">{(item.grossWeight * item.qty).toFixed(3)} g</span>
                        </div>
                      </div>
                    ))}

                    <div className="item-cards-totals">
                      <span>TOTALS</span>
                      <span>{totals.qty} pcs · <b style={{ color: 'var(--gold)' }}>{totals.grossWt.toFixed(3)} g</b></span>
                    </div>
                  </div>
                </>
              )}
